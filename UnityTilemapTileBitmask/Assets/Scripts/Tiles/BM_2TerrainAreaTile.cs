...
namespace UnityEngine.Tilemaps {
    ...
    [Serializable]
    public class BM_2TerrainAreaTile:TileBase {
        ...
        private int NeighborsToTileIndexTerrain(int mask) {
            ...
            switch (mask) {
                ...
                case 470: return 14;    // n ne c e s se
...
#if UNITY_EDITOR
        [MenuItem("Assets/Create/BM_2TerrainAreaTile")]
        public static void CreateBM_2TerrainAreaTile() {
            string path = EditorUtility.SaveFilePanelInProject("Save BitMask Terrain Area Tile", "New 9-bit Bitmask Tile", "asset", "Save BitMask Terrain Area Tile", "Assets");
            if (path == "") {
                return;
            }

            AssetDatabase.CreateAsset(ScriptableObject.CreateInstance<BM_2TerrainAreaTile>(), path);
        }
#endif
    }

#if UNITY_EDITOR
    [CustomEditor(typeof(BM_2TerrainAreaTile))]
    [CanEditMultipleObjects]
    internal class BM_2TerrainAreaTileEditor:Editor {
        private BM_2TerrainAreaTile tile { get { return (target as BM_2TerrainAreaTile); } }
        private const int k_bitSpriteCount = 15;
        ...
        private static Texture2D[] spriteIcons {
            get {
                if (s_spriteIcons == null) {
                    ...
                    s_spriteIcons[14] = Base64ToTexture("iVBORw0KGgoAAAANSUhEUgAAABAAAAAQAQMAAAAlPW0iAAAAA1BMVEX7AP8z6U5mAAAAC0lEQVQI12MgEQAAADAAAWV61nwAAAAASUVORK5CYII=");
                }
